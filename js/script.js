...
//Sorulara fotoğraf ve video ekleme 
    ...
    if (questions[index].soru_num === 1) {
        Soru = '<span>'+ questions[index].soru_num + ". " + questions[index].question +'<br>'+'</span>';
...
//optionSelected çalıştığında kullanılan func
function optionSelected(cevap){
    ...
    window.setTimeout(auto_past, 1000)
}
auto_past = ()=>{
...
function Soru_sayaci(index){
    let totalQueCounTag = '<span>Doğru Sayısı: <p>' +puan + '</p> / <p>'+ index +' </p> </span>';
